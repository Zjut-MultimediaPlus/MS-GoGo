...
def geo_transform(year,month,day,loc_y,loc_x,L,W,step,patch_l,patch_w,range_l,range_w,start_lat,start_lon):
    ...
    output = torch.concat((year_time, y_result, x_result, l_result, w_result)).cuda()

    return output #(year_time, y_result, x_result, l_result, w_result)

class MLC_Dataset_16c_gpu(torch.utils.data.Dataset):


    def __init__(self, csv_file, root_dir, num_labels ,known_labels=0,transform=None,testing=False,tk_ratio=0.25,device=torch.device('cuda:0')):
        """
        Args:
        ...
        self.whole_hsi = torch.from_numpy(self.whole_hsi).to(device)
        self.ymd_str = '20190101'
        self.max,_ = torch.max(self.whole_hsi.reshape(16,-1),dim=1)
        self.max = self.max.reshape(16,1,1)
        self.min,_ = torch.min(self.whole_hsi.reshape(16,-1),dim=1)
        self.min = self.min.reshape(16,1,1)
    ...
    def __getitem__(self, idx):
        ...
        if patch_ymd != self.ymd_str:
            ...
            self.whole_hsi = np.load(self.file_path)
            self.whole_hsi = torch.from_numpy(self.whole_hsi).to(self.device)
            self.max, _ = torch.max(self.whole_hsi.reshape(16, -1), dim=1)
            self.max = self.max.reshape(16, 1, 1)
            self.min, _ = torch.min(self.whole_hsi.reshape(16, -1), dim=1)
            self.min = self.min.reshape(16, 1, 1)
