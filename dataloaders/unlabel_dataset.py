...
class Unlabel_Dataset_16c(torch.utils.data.Dataset):


    def __init__(self, csv_file, root_dir, testing=False):
        """
        Args:
        ...
        self.max_1 = self.whole_hsi_1.reshape(16,-1).max(axis=1).reshape(16,1,1)
        self.min_1 = self.whole_hsi_1.reshape(16,-1).min(axis=1).reshape(16,1,1)
        self.whole_hsi_1 = (self.whole_hsi_1-self.min_1)/(self.max_1-self.min_1)

        self.max_2 = self.whole_hsi_2.reshape(16, -1).max(axis=1).reshape(16, 1, 1)
        self.min_2 = self.whole_hsi_2.reshape(16, -1).min(axis=1).reshape(16, 1, 1)
    ...
    def __getitem__(self, idx):
        ...
        '''
        do with the model forwarding
        if not self.testing:
            if np.min(sample_1.min()<0):
                print(sample_1.min())

            #sample_1 = utils.augmentation.data_augmentation(sample_1)
            sample_1 = utils.aug_plus.augmentation_plus(sample_1.copy()).copy()
            sample_2 = utils.aug_plus.augmentation_plus(sample_2.copy()).copy()
            #
            # print(hsi.shape)
        ...
        sample_1 = torchvision.transforms.Resize(256)(sample_1).detach()
        sample_2 = torch.from_numpy(sample_2.astype('float32'))
        sample_2 = torchvision.transforms.Resize(256)(sample_2).detach()
        ...
        if not self.testing:
            ...
            if flag == 1:
                sample_1 = utils.aug_plus.augmentation_plus_gpu(sample_1)
            else:
                sample_2 = utils.aug_plus.augmentation_plus_gpu(sample_2)
        ...
        out_version_1 = {'image': sample_1, 'time_and_loc': time_and_loc}
        out_version_2 = {'image': sample_2, 'time_and_loc': time_and_loc}
...
class Unlabel_Dataset_16c_with_loc_and_month(torch.utils.data.Dataset):
    ...
    def __init__(self, csv_file, root_dir, testing=False):
        ...
        # file_path_list
        random_flag_1 = np.random.randint(low=0, high=5, size=1)
        self.file_path_1 = self.root_dir + '/2018_00%1d0/NC_H08_20180101_00%1d0_R21_FLDK.02401_02401.nc' % (
        random_flag_1, random_flag_1)
        self.whole_hsi_1 = read_rs_to_numpy(self.file_path_1)

        random_flag_2 = np.random.randint(low=0, high=5, size=1)
        self.file_path_2 = self.root_dir + '/2018_00%1d0/NC_H08_20180101_00%1d0_R21_FLDK.02401_02401.nc' % (
        random_flag_2, random_flag_2)
        self.whole_hsi_2 = read_rs_to_numpy(self.file_path_2)
    ...
    def __getitem__(self, idx):
        ...
        shift_y = 0#int(np.random.uniform(0, 80, size=None))
        y_end = min(2401, sample_y * step + w + shift_y)
        shift_x = 0#int(np.random.uniform(0, 80, size=None))
        x_end = min(2401, sample_x * step + w + shift_x)
        ...
        out_version_1['loc_num'] = (sample_y) * 11 + (sample_x)
        # print(img_loc,sample['loc_num'])
        out_version_1['month'] = sample_month - 1

        out_version_2['loc_num'] = (sample_y) * 11 + (sample_x)
        # print(img_loc,sample['loc_num'])
        out_version_2['month'] = sample_month - 1
