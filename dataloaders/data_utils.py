...
def get_unk_mask_indices(image,testing,num_labels,known_labels,epoch=1,tk_ratio=0.25): #zdxy tk_ratio train_known_ratio
    if testing:
        ...
        unk_mask_indices = range(num_labels) #random.sample(range(num_labels), (num_labels-int(known_labels)))
    else:
        # sample random number of known labels during training
        if known_labels>0:
            random.seed()
            num_known = random.randint(0,int(num_labels*(1-tk_ratio)))
        else:
            num_known = 0
