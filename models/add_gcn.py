...
class ADD_GCN(nn.Module):
    def __init__(self, backbone:str ,model, num_classes, hidden):
        super(ADD_GCN, self).__init__()
        if 'res' in backbone:
            self.features = nn.Sequential(
                torch.nn.Conv2d(16,64,kernel_size=(7,7),stride=(2,2),padding=(3,3),bias=False),#model.conv1,
                model.bn1,
                model.relu,
                model.maxpool,
                model.layer1,
        ...
        if 'res' in backbone:
            ...
            self.fc = nn.Conv2d(512, num_classes, (1, 1), bias=False)

        # zdxy: to match the channels of backbone
        self.conv_transform = nn.Conv2d(hidden, 1024, (1, 1))
        self.relu = nn.LeakyReLU(0.2)
