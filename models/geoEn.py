...
from .backbone import Backbone, Backbone_split_res101, Backbone_split_res34, Backbone_split_res18, Backbone_split_res50, \
    CNN
...
class GeoTransEn_16c(nn.Module):
    def __init__(self, args, num_labels, layers=3, heads=4, dropout=0.1):
        super(GeoTransEn_16c, self).__init__()
        ...
        self.trans = nn.Transformer(hidden, heads, args.enc_layers, args.dec_layers, hidden, dropout, norm_first=False,
                                    batch_first=True)
        self.encoder_layer = nn.TransformerEncoderLayer(hidden, heads, hidden, dropout, batch_first=True,
                                                        norm_first=True)
        self.encoder = nn.TransformerEncoder(self.encoder_layer, args.enc_layers, nn.LayerNorm(hidden))
        self.decoder_layer = nn.TransformerDecoderLayer(hidden, heads, hidden, dropout, batch_first=True,
                                                        norm_first=True)
    ...
    def forward(self, images, sl, train=True):
        ...
        year_time, y, x, l, w = torch.zeros(self.bs, 1).cuda(), torch.zeros(self.bs, 1).cuda(), torch.zeros(self.bs,
                                                                                                            1).cuda(), torch.zeros(
            self.bs, 1), torch.zeros(self.bs, 1)
        ...
        embeddings = torch.cat((img_embedding,label_embeddings),dim=1)
        ...
        enhenced_embeddings = self.encoder(embeddings)
        assert torch.sum(torch.isnan(enhenced_embeddings)) == 0, print(enhenced_embeddings)


        img_rep = self.enhence_feature_projector(self.enhence_feature_norm(enhenced_embeddings[:,:-self.num_labels,:]))
        assert torch.sum(torch.isnan(img_rep)) == 0, print(img_rep)
        out_seq = enhenced_embeddings[:,-self.num_labels:,:]
    ...
    def geo_transform(self, year, month, day, loc_y, loc_x, L, W, step, patch_l, patch_w, range_l, range_w, start_lat,
                      start_lon):
        '''
        ...
        month_prior_leap = [0, 31, 59 + 1, 90 + 1, 120 + 1, 151 + 1, 182 + 1, 213 + 1, 244 + 1, 274 + 1, 305 + 1,
                            335 + 1]
